<?php
   ...
   */
	...
	const socialTable = "settings_social";
    ...
    {
      $this->getSettings();
      $this->getSettingsSocial();
      ($this->dtz) ? date_default_timezone_set($this->dtz) : date_default_timezone_set('America/Toronto');
    ...
    private function getSettings()
    {
      ...
      $this->support_email = $row->support_email;
      $this->payment_email = $row->payment_email;
      $this->etransfer_answer = $row->etransfer_answer;
      $this->seo = $row->seo;
      ...
      $this->shipstation_api = $row->shipstation_api;

      $this->shipping_standard = $row->shipping_standard;
      $this->shipping_express = $row->shipping_express;
      $this->shipping_free_flag = $row->shipping_free_flag;
      $this->meltable_warning = $row->meltable_warning;
      $this->invite_only = $row->invite_only;
    ...
    /**
	 * Core::getSettingsSocial()
	 *
	 */
	private function getSettingsSocial() {
		$sql = "SELECT * FROM " . self::socialTable;
		$row = Registry::get("Database")->first($sql);
		$this->social_facebook = $row->social_facebook;
		$this->enable_facebook = $row->enable_facebook;
		$this->social_twitter = $row->social_twitter;
		$this->enable_twitter = $row->enable_twitter;
		$this->social_instagram = $row->social_instagram;
		$this->enable_instagram = $row->enable_instagram;
		$this->social_youtube = $row->social_youtube;
		$this->enable_youtube = $row->enable_youtube;
		$this->social_soundcloud = $row->social_soundcloud;
		$this->enable_soundcloud = $row->enable_soundcloud;
		$this->social_pinterest = $row->social_pinterest;
		$this->enable_pinterest = $row->enable_pinterest;
	}
	 ...
	 *
	 */
  	public function processConfigSocial() {
      ...
      $data = array(
        ...
        'social_soundcloud' => sanitize($_POST['social_soundcloud']),
        'social_pinterest' => sanitize($_POST['social_pinterest']),
        'enable_facebook' => intval($_POST['enable_facebook']),
        'enable_twitter' => intval($_POST['enable_twitter']),
        'enable_instagram' => intval($_POST['enable_instagram']),
        'enable_youtube' => intval($_POST['enable_youtube']),
        'enable_soundcloud' => intval($_POST['enable_soundcloud']),
        'enable_pinterest' => intval($_POST['enable_pinterest'])
      );

      Registry::get("Database")->update(self::socialTable, $data);
      ...
      if (Registry::get("Database")->affected()) {
        ...
        $json['type'] = 'success';
        $json['message'] = Filter::msgAlert(Lang::$word->NOPROCESS, false);
      }
     ...
     */
      ...
      //Load settings Search Object
      
      /*
      global $searcher;
      $searcher->settings->setIgnoredWordsFromString(post('search-ignored-words'));
      $searcher->settings->setFreebirdIgnoredWordsFromString(post('search-freebird-ignored-words'));
      $searcher->settings->setMethod(post('search-method'));
      $searcher->settings->setWeight('title', post('search-weight-title'));
      $searcher->settings->setWeight('artist', post('search-weight-artist'));
      $searcher->settings->setWeight('transcriber', post('search-weight-transcriber'));
      $searcher->settings->setWeight('description', post('search-weight-description'));
      $searcher->settings->ignoreLowerScorePercentage = (int)post('search-ignore-percentage');
      $searcher->settings->demoteLowerScorePercentage= (int)post('search-demote-percentage');
      $searcher->settings->minScore = (int)post('search-min-score');
      $searcher->settings->useIgnoredWords = (bool)post('search-use-ignored-words');

      if (!$searcher->saveSettings()){
          $json['type'] = 'failed';
          $json['message'] = "Error saving Search Settings.";
          print json_encode($json);
          return;
      }

      if (post('search-reindex-all')) {
          $r = array();
          $searcher->indexAll($r);
      }

      //Scroll settings
      global $scrollSettings;
      $scrollSettings->perPage = post('paging-per-page');
      global $db;
      if (!$db->saveCustomSettings(\paging\PagingSettings::SETTINGS_KEY, json_encode($scrollSettings))){
          $json['type'] = 'failed';
          $json['message'] = "Error saving Scroller Settings.";
          print json_encode($json);
          return;
      }
      ...
      */
      ...
      //This checks to see if a value is entered.
  		...
  		*/
  		...
  		if (!empty($_FILES['logo']['name'])) {
  			...
  			if (empty($file_info)) Filter::$msgs['logo'] = Lang::$word->CONF_LOGO_R;
  		}
  		...
  		if (empty(Filter::$msgs)) {
  			$data = array(
				...
				'shipping_standard' => floatval($_POST['shipping_standard']),
				'shipping_express' => floatval($_POST['shipping_express']),
				'shipping_free_flag' => floatval($_POST['shipping_free_flag']),
				'meltable_warning' => intval($_POST['meltable_warning']),
				'invite_only' => intval($_POST['invite_only'])
  			);
	  ...
	  {
		  return "<div class=\"black bottom right attached special label\">" . strtoupper(self::$language) . "</div>";
	  }
	  ...
	  public function formatMoney($amount, $free = true)
	  {
		  ...
		  return ($amount == 0) ? $is_free : $this->cur_symbol . number_format($amount, 2, '.', ',');
	  }
