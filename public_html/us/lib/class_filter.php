...
final class Filter
{
	...
	public static function msgAlert($msg, $print = true, $fader = false, $altholder = false)
	{
		...
		self::$showMsg = '<div class="kode-alert alert11 kode-alert-bottom-right" style="display: block;"> <h4>' . Lang::$word->ALERT . '</h4> ' . $msg . '</div>';

		if ($fader == true) self::$showMsg.= "<script type=\"text/javascript\">
	...
	{
		self::$showMsg = "<div class=\"warning message\"><i class=\"attention icon\"></i> " . $msg . "</div>";
		if ($print == true) {
			print self::$showMsg;
		}
		else {
	 ...
	 * @return
	 */
	public static function msgSingleAlertPopup($msg, $print = true)
	{
		self::$showMsg = '<script class="jbox-popuptemp" type="text/javascript">';
		self::$showMsg.= "var options = {content: '" . $msg . "', autoClose: 3400, attributes: {x: 'right', y: 'bottom'}};";
		self::$showMsg.= "new jBox('Notice', options);";
		self::$showMsg.= '</script>';
		if ($print == true) {
			print self::$showMsg;
		}
	 ...
	 * @return
	 */
	public static function msgOkSimple($msg)
	{
		self::$showMsg = '<div class="message-wrapper fadeIn animated">';
		self::$showMsg.= '<p class="messages">' . $msg . '</p>';
		self::$showMsg.= '</div>';
		return self::$showMsg;
	}
	...
	{
		self::$showMsg = '<div class="kode-alert alert3 kode-alert-bottom-right" style="display: block;"> <h4>' . Lang::$word->SUCCESS . '</h4> ' . $msg . '</div>';
		...
		if ($fader == true) {
			...
			self::$showMsg.= "// <![CDATA[";
			self::$showMsg.= "setTimeout(function() { $(\".msgOk\").fadeOut(\"slow\", function() { $(\".msgOk\").remove(); }); }, 4000);";
			self::$showMsg.= "// ]]>";
			self::$showMsg.= "</script>";
		}
	...
	{
		self::$showMsg = "<div class=\"success message\"><i class=\"ok sign icon\"></i> " . $msg . "</div>";
		if ($print == true) {
			print self::$showMsg;
		}
		else {
	 ...
	 * @return
	 */
	public static function msgSingleOkPopup($msg, $print = true)
	{
		self::$showMsg = '<script class="jbox-popuptemp" type="text/javascript">';
		self::$showMsg.= "var options = {content: '" . $msg . "', autoClose: 3400, attributes: {x: 'right', y: 'bottom'}, addClass:'success'};";
		self::$showMsg.= "new jBox('Notice', options);";
		self::$showMsg.= '</script>';
		return self::$showMsg;
	}
	...
	{
		self::$showMsg = "<div class=\"icon message info\"><i class=\"flag icon\"></i><i class=\"close icon\"></i><div class=\"content\"><div class=\"header\"> " . Lang::$word->INFO . "</div><p>" . $msg . "</p></div></div>";
		if ($fader == true) self::$showMsg.= "<script type=\"text/javascript\">
	...
	{
		self::$showMsg = "<div class=\"info message\"><i class=\"information icon\"></i> " . $msg . "</div>";
		if ($print == true) {
			print self::$showMsg;
		}
		else {
	...
	{
		self::$showMsg = "<div class=\"icon message error\"><i class=\"flag icon\"></i><i class=\"close icon\"></i><div class=\"content\"><div class=\"error-message\"> " . Lang::$word->ERROR . "</div><p>" . $msg . "</p></div></div>";
		if ($fader == true) self::$showMsg.= "<script type=\"text/javascript\">
	...
	{
		self::$showMsg = "<div class=\"error message\"><i class=\"ban circle icon\"></i> " . $msg . "</div>";
		if ($print == true) {
			print self::$showMsg;
		}
		else {
	...
	{
		self::$showMsg = "<div class=\"error message\"><i class=\"close icon\"></i><div class=\"header\">" . Lang::$word->PROCCESS_ERR . "</div><div class=\"content\"><ul class=\"list\">";
	...
	/**
	 * Filter::msgStatusSimple()
	 *
	 * @return
	 */
	public static function msgStatusSimple()
	{
		self::$showMsg = '<div class="message-wrapper error fadeIn animated">';
		$i = count(self::$showMsg);
		foreach(self::$msgs as $msg) {
			self::$showMsg.= '<p class="messages error">' . $msg . '</p>';
		}
	...
	/**
	 * Filter::msgStatusPopup()
	 *
	 * @return
	 */
	public static function msgStatusPopup()
	{
		self::$showMsg = '<script class="jbox-popuptemp" type="text/javascript">';
		$i = count(self::$showMsg);
		foreach(self::$msgs as $msg) {
			self::$showMsg.= "var options = {content: '" . $msg . "', autoClose: 3400, attributes: {x: 'right', y: 'bottom'}, addClass:'error'};";
			self::$showMsg.= "new jBox('Notice', options);";
		}
	...
	private static function getAction()
	{
		if (isset(self::$get['action'])) {
			...
			else return self::$action = $action;
		}
	}
	...
	{
		if (empty($_POST[$index])) self::$msgs[$index] = $msg;
	}
