...
<?php
	$paypal_cartrow = $content->getWholesaleCartContent();
					...
					<div class="cart-wrapper receipt t30">
						...
						<div id="normalpay-display" class="receipt-summary" style="max-width: 320px;">
							...
							<div class="table receipt-sect">
								...
								<div class="trow t-pad-bot tax-row">
					...
					<div class="cart-wrapper">
						...
						<div class="row payment-options">
							...
							<div class="col-payment col-sm-4 p30">
								<a class="wholesale-payment-option active" data-payment="cashpayment" id="pointspayment-opt">
									<div class="card-options">
										<img src="<?php echo THEMEURL;?>/assets/img/icons/local_seo.png" alt="" />
							...
							<div class="col-payment col-sm-4 p30">
								<a class="wholesale-payment-option link" data-payment="stripepayment" id="stripepayment-opt">
									<div class="card-options">
							...
							<div class="col-payment col-sm-4 p30">
								<a class="wholesale-payment-option link" data-payment="etransferpayment">
									<div class="card-options">
							...
							<!--TODO: Paypal receipt -->
							<div class="col-payment col-sm-4 p30" style="display:none">
						...
						<!-- Cash on Delivery -->
						<div class="payment-info cashpayment active">
							<form method="post" action="gateways/cash/wholesale_ipn.php">

								<h5 class="p18 t18 bold-text">Cash on Delivery</h5>
								
								
								<p style="max-width: 522px;">
									To schedule a pickup text or call 647-847-6557 or send us an email <a href="mailto:hi@budabomb.com">hi@budabomb.com</a>.
								</p>
								...
								<input type="hidden" name="processCashPayment" value="1" />
						...
						<div class="payment-info etransferpayment">
							<form method="post" action="gateways/etransfer/wholesale_ipn.php">
								...
								<p style="max-width: 522px;">
									...
									If you are new to Interac E-transfers, please visit <a href="http://www.interac.ca/en/interac-e-transfer-consumer.html" target="_blank">http://www.interac.ca/en/interac-e-transfer-consumer.html</a> to read all information.
								</p>
						...
						<!-- Stripe -->
						<div class="payment-info stripepayment">

							<h5 class="p18 t18 bold-text">Credit / Debit Card</h5>
							

							<form method="post" id="stripe_form" class="stripepayment" action="gateways/stripe/wholesale_ipn.php">
								<input type="hidden" name="amount" value="<?php echo $cart->totalprice;?>" />
								<input type="hidden" name="user_email" value="<?php echo $urow->username;?>" />
								<input type="hidden" name="user_id" value="<?php echo $user->uid;?>" />
								<input type="hidden" name="item_name" value="<?php echo $itemDetails;?>" />
								<input type="hidden" name="currency_code" value="CAD" />
								<input type="hidden" name="discount_code" value="<?php echo($cart->discount_code); ?>" />
								<input type="hidden" name="processWholeSaleStripePayment" value="1" />
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    $('.btn-quantum').click(function(e) {
	        ...
	        $.ajax({
	            type: 'post',
	            url: "ajax/check-wholesale-product-in-stock.php",
	            dataType: 'json',
	            data: {
	            },
		...
		$( ".payment-options" ).on( "click", ".wholesale-payment-option", function() {
		$(".wholesale-payment-option").removeClass("active");
		$(".payment-info").removeClass("active");

		var paymentType = "." + $(this).data('payment');
		$(paymentType).addClass("active");
		$(this).addClass("active");
		if(".stripepayment" == paymentType){
			var gtotal = parseFloat($('#gtotal').text().replace('$','').replace(',',''));
			var serviceFee = gtotal*0.02;
			gtotal = (gtotal + serviceFee).toFixed(2);
			$('<div class="trow t-pad-bot service-fee-row"><div class="cell">Service Fee:</div><div id="service-fee" class="cell">$'+serviceFee.toFixed(2)+'</div></div>').insertAfter(".tax-row");
			$('#gtotal').text(formatter.format(gtotal));
			$('.tax-row').removeClass("t-pad-bot");
		}else{
			var serviceFee = 0;
			if ($("#service-fee").length ) {
			 serviceFee = parseFloat($("#service-fee").text().replace('$','').replace(',',''));
			}
			var gtotal = parseFloat($('#gtotal').text().replace('$','').replace(',',''));
			gtotal = (gtotal - serviceFee).toFixed(2);
			$('#gtotal').text(formatter.format(gtotal));
			$(".service-fee-row").remove();
			$('.tax-row').addClass("t-pad-bot");
		}
		});
	});
