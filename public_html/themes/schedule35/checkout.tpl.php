...
<body id="page-checkout" class="<?php echo (($notification == 1) ? 'has-notification' : ''); ?>">
	...
	<script type="text/javascript">
		...
		$('#shipping_state').on('change', function(){
			...
			if (shippingFree == 1) {
				$("#shipping_economy_dis").text("FREE");
				$("#shipping_express_dis").text("$" + (<?php echo $core->shipping_express;?> + shipping_province_cost).toFixed(2));
			} else {
				$("#shipping_economy_dis").text("$" + (<?php echo $core->shipping_standard;?> + shipping_province_cost).toFixed(2));
				$("#shipping_express_dis").text("$" + (<?php echo $core->shipping_express;?> + shipping_province_cost).toFixed(2));
			}
			updateShippingCost();
