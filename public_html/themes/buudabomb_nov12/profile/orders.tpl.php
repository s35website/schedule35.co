...
<div id="profile-orders" class="wrapper padded bg-lightgrey">
	...
	<div class="container max-width narrow page details">
		...
		<div class="empty">
			...
			<a class="btn med btn primary" href="<?php echo SITEURL;?>/shop">Shop <?php echo $core->company; ?>'s</a>
		</div>
		...
		<div class="form-validetta masta order-history" data-id="<?php echo($row->id); ?>">
			<div class="tab view">
				<h2 class="p0">
					...
					<?php if($row->pp == "BuudaPoints"):?>
					<?php echo number_format($row->totalprice, 0);?> pts
					<?php else:?>
					$<?php echo money_format('%i', $row->totalprice);?>
					<?php endif;?>
				</h2>
					...
					<div class="col-sm-12">
						...
						<p>
							<?php
								...
								elseif ($row->status == 3) {
									$status = Lang::$word->SHIP_3 . " (Tracking <a target='_blank' href='https://www.canadapost.ca/cpotools/apps/track/personal/findByTrackNumber?trackingNumber=" . $row->trackingnum . "'>#" . $row->trackingnum . "</a>)";
								}
								else {
