...
if (isset($_POST['processStripePayment'])) {
	...
	if (intval($_POST['billing_address_opt'])) {
		$bill_country = sanitize($_POST['billing_lname']);
	...
	try {
		$charge = Stripe_Charge::create(array(
			...
			"card" => array(
				...
				'address_line2' => $bill_address2,
				'address_zip' => $bill_zip,
				'address_state' => $bill_state,
				'address_country' => $bill_country),
			"description" =>  sanitize($_POST['user_email']) . " | " . $_POST['address'] . ": Purchases: " . $_POST['item_name']));
		...
		if ($chargeArray['paid']==1 && $v1 <= $v2) {
			...
			header("location:../../receipt?type=visa&tx=" . sanitize($txn_id));
			exit();
	    }
	}
